...
def run_simulation(graph, gamma, beta):
    ...
    with torch.no_grad():
        for t in tqdm(range(1, ITERS + 1), desc=f"Simulating Gamma={gamma}, Beta={beta}", leave=False, mininterval=1.0):
...
def run_single_config(args):
    c_name, graph, g, beta, graph_titles_c_name = args
    p_hist, qc_hist, qd_hist = run_simulation(graph, gamma=g, beta=beta)
    out_file = os.path.join(out_dir, f"{c_name}_gamma{g}_beta{beta}.jpg")
    title = f"{graph_titles_c_name}, Gamma={g}, Beta={beta}"
    plot_dynamics(p_hist, qc_hist, qd_hist, title, out_file, c_name)
    return True

def main():
    print("Generating Graphics for Beta=1 and Beta=2...")
    configurations = {
    ...
    for c_name, graph in configurations.items():
        for g in GAMMAS:
            for b_val in BETAS:
                tasks.append((c_name, graph, g, b_val, graph_titles[c_name]))
